var myCalendar = (function () {
    ...
    var run = function (parent, isTodayBtn, size, callback) {
        // 달력생성
        var calendar = {
            parent: parent, //달력이 위치할 부모노드
            viewDay: {}, //달력에 보여지는 날짜
            selectedDay: {}, //사용자가 클릭한 날짜
            tdArr: [], //달력테이블셀
            isTodayBtn: isTodayBtn, //오늘버튼 생성여부
            size: size, //달력사이즈
        }
    ...
    var checkParameter = function(arguments) {
        if (!arguments[0]) {
            throw new Error ('first parameter is empty.')
        } else if (arguments[1] && typeof arguments[1] !== 'boolean') {
            throw new Error ('second parameter is not a Boolean type.');
        } else if (arguments[2] && typeof arguments[2] !== 'function') {
            throw new Error ('third parameter is not a function.');
        }
    }

    var create = function (parentNode, isTodayBtn, callback) {
        checkParameter(arguments);
        var isTodayBtn = arguments.length > 1 ? arguments[1] : false;
        var callback = arguments.length > 2 ? arguments[2] : undefined;
        return run(parentNode, isTodayBtn, 'big', callback);
    }

    var createMini = function (parentNode, isTodayBtn, callback) {
        checkParameter(arguments);
        var isTodayBtn = arguments.length > 1 ? arguments[1] : false;
        var callback = arguments.length > 2 ? arguments[2] : undefined;
        return run(parentNode, isTodayBtn, 'mini', callback);
    }
